...
This script supports running on Termux (Android terminal emulator).

### Setup on Termux

1. **Enable X11 repository and update packages:**
...
2. **Install required dependencies:**
   ...
   ```bash
   pkg install nodejs-lts git chromium
   ```
...
3. **Verify Chromium installation:**
   ...
   Expected output: `/data/data/com.termux/files/usr/bin/chromium-browser`

4. **Clone and configure:**

   ```bash
   git clone https://github.com/aihara87/Microsoft-Rewards-Script.git
   cd Microsoft-Rewards-Script
   cp src/accounts.example.json src/accounts.json
   nano src/accounts.json  # Add your credentials
   ```

5. **Install and build:**

   ```bash
   npm run pre-build:termux
   npm run build
   ```
...
| ---------------------------- | ------------------------------------------------------------- |
| `PLAYWRIGHT_BROWSERS_PATH=0` | Disables automatic browser downloads (used by `start:termux`) |
| `CHROMIUM_PATH`              | Custom path to Chromium executable (auto-detected if not set) |

### Running in Background

To keep the script running when Termux is in background:
